	...
	float YawOverride)
{
	...
	for (USceneComponent* Pt : ImmutablePts)
	{
		...
		const FVector AdjustedLocation = FVector(RaisedLocation.X, RaisedLocation.Y, HitResult.ImpactPoint.Z);
		Pt->SetWorldLocation(AdjustedLocation);
		// ImpactNormal碰撞表面的法线向量， MakeRotFromZ以该法线向量作为Z轴进行旋转，使Pt贴合于斜面
