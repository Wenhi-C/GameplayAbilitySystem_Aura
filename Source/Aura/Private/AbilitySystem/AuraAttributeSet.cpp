	...
	if (Data.EvaluatedData.Attribute == GetIncomingXPAttribute())
	{
		...
		if (Props.SourceCharacter->Implements<UPlayerInterface>())
		{
			...
			if (NumLevelUps > 0)
			{
				...
				{
					AttributePointsReward += IPlayerInterface::Execute_GetAttributePointsReward(Props.SourceCharacter, UpLevel);
					SpellPointsReward += IPlayerInterface::Execute_GetSpellPointsReward(Props.SourceCharacter, UpLevel);
				}
...
void UAuraAttributeSet::SendXPEvent(const FEffectProperties& Props) const
{
	if (Props.TargetCharacter->Implements<UCombatInterface>())
	{
		...
		const int32 XPReward = UAuraAbilitySystemLibrary::GetXPRewardForClassAndLevel(this, TargetCharacterClass, TargetLevel);
